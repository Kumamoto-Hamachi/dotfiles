...
migration_cook() {
	...
	if [ $order = "1" ]; then
		...
		if [ $app_name = "any" ]; then
			local order_code="no.2 python manage.py showmigrations"
			python manage.py showmigrations
		else
			local order_code="no.2 python manage.py showmigrations $app_name"
		...
		local order_code="no.3 python manage.py migrate $app_name $migration_name"
